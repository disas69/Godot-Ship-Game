...
func spawn_players_from_game_mode() -> void:
	...
	if selected_game_mode == null:
		push_warning("Game: game_mode is not assigned.")
		return
...
func spawn_local_player(team: Ship.Team, local_player_index: int, control_scheme: String = PlayerInput.CONTROL_KEYBOARD) -> PlayerShip:
	...
	if player_instance == null:
		push_warning("Game: local_player_scene is not a PlayerShip scene.")
		return null
...
func spawn_bot_player(team: Ship.Team) -> BotShip:
	if bot_player_scene == null:
		push_warning("Game: bot_player_scene is not assigned.")
		return null

	var player_instance: BotShip = bot_player_scene.instantiate() as BotShip
	if player_instance == null:
		push_warning("Game: bot_player_scene is not a BotShip scene.")
		return null
...
func get_next_spawn_point(team: Ship.Team) -> Node3D:
	...
	if spawn_points.is_empty():
		push_warning("Game: missing spawn points for team %s." % [team])
		return null
